...
module Cardano.PlutusCode.SellBuyContract
    ( SellOffer (..)
    , SellParams (..), BuyOfferParams (..), CloseParams (..)
    , SellOfferSchema
    , sell, buy, close,test
    , endpoints
    , schemas
...
mkSellOfferValidator ad redeemer ctx =
    ...
    case redeemer of
        MkBuyOffer b@BuyOffer{..} ->
            ...
            traceIfFalse "wrong output address"        (correctBuyOfferOutputAddress)   &&
        ...
        Close           -> 
            case adBuyOffer ad of
                ...
                Just b@BuyOffer{..} ->
                    (traceIfFalse "seller's signature missing" signedBySeller ||
                    traceIfFalse "buyer's signature missing" (signedByBuyer b)) &&
  ...
  where
    ...
    signedBySeller :: Bool
    signedBySeller =txSignedBy info $ (unPaymentPubKeyHash (aSeller (adSellOffer ad))) 

    signedByBuyer :: BuyOffer -> Bool
    signedByBuyer x= txSignedBy info $ (unPaymentPubKeyHash (bBuyer x)) 
    ...
    correctBuyOfferOutputAddress :: Bool
    correctBuyOfferOutputAddress =
        txOutAddress ownOutput == txOutAddress ((txInfoOutputs info) !! 0)
  --  correctBuyOfferOutputAddress :: BuyOffer -> Bool
  --  correctBuyOfferOutputAddress bO =
  --      txOutAddress ownOutput == sellOfferAddress
...
buy BuyOfferParams{..} = do
    ...
    when (bpBuyOffer < minBuyOffer d) $
        Contract.throwError $ pack $ printf "buy offer lower than minimal buy offer %d" (minBuyOffer d)
    pkh <- ownPaymentPubKeyHash
    let b  = BuyOffer {bBuyer = pkh, bBuyOffer = bpBuyOffer}
        d' = d {adBuyOffer = Just b}
        v  = Value.singleton bpCurrency bpToken 1 <> Ada.lovelaceValueOf (minLovelace + bpBuyOffer)
        ...
        tx      = Constraints.mustPayToTheScript d' v                            <>
            ...
            --                         Constraints.mustSpendScriptOutput oref r
                    -- Just BuyOffer{..} -> Constraints.mustPayToTheScript d' v                            <>
                    --                 Constraints.mustPayToPubKey bBuyer (Ada.lovelaceValueOf bBuyOffer) <>
                    --                 Constraints.mustSpendScriptOutput oref r
        ...
        [(oref, o)] -> case _ciTxOutDatum o of
            ...
            Right (Datum e) -> case PlutusTx.fromBuiltinData e of
                ...
                Just d@SellOfferDatum{..}
                    ...
                    | otherwise                                           -> Contract.throwError "sellOffer token missmatch"
        _           -> Contract.throwError "sellOffer utxo not found"
...
emCfg = EmulatorConfig $ Left $ Map.fromList $[(Wallet w, v) | w <- [1 .. 3]]
  where
    v :: Value
    v = Ada.lovelaceValueOf 1000000000 <> assetClassValue token 1
...
myTrace=do 
    ...
    callEndpoint @"sell" h1 $ SellParams
        { spSellPrice  = 10000000
